...
struct CommandValidationTests {
  ...
  func mapsLoadProduct() throws {
    let temporaryURL = FileManager.default.temporaryDirectory
      .appending(path: "playdate-simctl-\(UUID().uuidString).pdx", directoryHint: .isDirectory)
    try FileManager.default.createDirectory(at: temporaryURL, withIntermediateDirectories: false)
    defer { try? FileManager.default.removeItem(at: temporaryURL) }

    #expect(try Load.parse([temporaryURL.path]).simulatorCommand == .load(path: temporaryURL.path))
  }
